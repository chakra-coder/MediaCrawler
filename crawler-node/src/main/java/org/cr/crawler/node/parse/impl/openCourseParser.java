...
/**
 * Author: caorong Date: 13-11-16 Time: 下午10:34 To change this template use File
 * | Settings | File Templates.
 */
public class openCourseParser extends AbstractParser implements Parser {
	...
	static {
		...
		site.setDomain("http://open.163.com/");
		// timeout 100s
		site.setTimeOut(100 * 1000);
	}

	@Override
	public List<Map<String, String>> getDetailUrls(Task task) throws Exception {
		...
		for (int i = 0; i < urlNodes.size(); i++) {
			...
			String sortNum = getSortNum(Xsoup.compile("//body/text()")
					.evaluate(snode).get());
